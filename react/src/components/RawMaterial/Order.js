...
const Order = (props) =>{
    ...
    const checkValue = ()=>{
        if(quantity === 0 || dateTime ===''|| (new Date(dateTime) < Date.now()) ){
            return false;
        }
        return true;
    }
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" classes={ classes.dialogWrapper }>
            ...
            <DialogActions classes={{root: classes.leftDialogActions}}>
                {(page ===0 ) &&<Button onClick={handleClose} color="primary">Cancel</Button>}
                {(page ===0 ) &&<Button onClick={handleConfirm} color="primary">Confirm</Button>}
