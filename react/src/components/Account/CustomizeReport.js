...
const CustomizeReport = (props) =>{
    const {open, handleClose,setPage, setPayableList,setReceivableList} = props;
    ...
    const handleSubmitCustomize =()=>{
        let reportPath = reportType === "Sale"? "customer": (reportType === "Purchase"?"orders": "");
        if(reportPath === "" ){
            return;
        }
        request
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" classes={ classes.dialogWrapper }>
            ...
            <DialogActions classes={{ root: classes.leftDialogActions }}>
                ...
                <Button onClick={handleSubmitCustomize} color="primary">Submit</Button>
            </DialogActions>
        </Dialog>
    )
};
